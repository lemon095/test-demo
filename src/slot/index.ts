...
export default class BaseSlot<T extends Record<string, any>> {
   ...
   * @param { boolean } options.colLengthIsZero - 选填，列长度是否为 0，默认为 false（不为零）
  ...
  public getAdjoinWp({
    ...
    for (let i = 0; i < firstCol.length; i++) {
      ...
      if (lineCount > winCount) {
        winnerLineCount[icon] = lineCount;
      }
    }
  ...
  /**
   * nswl 百搭形态的初始信息，该字段只表示初始，不参与后续的形态变化逻辑
   * @param {Object} options - 配置信息
  ...
  public getNswl({
    ...
    // 再判断是否存在百搭中奖
    const wps = flattenDeep(values(wp));
